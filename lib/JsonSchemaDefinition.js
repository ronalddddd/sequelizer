...
class JsonSchemaDefinition extends ForeginDefinition {
  ...
  compileProperty(propName) {
    ...
    if (!isMixin) {
      modelDef[propName] = {};
      fieldDef = modelDef[propName];
    }
    // We're done if this field has a sequelize.VIRTUAL type definition provided
    if (customFieldDef) {
      modelDef[propName] = customFieldDef;
    } else if (referencedSchema || schemaProp.type === 'object' || schemaProp.type === 'array') {
      ...
      switch (schemaProp.type) {
        ...
        case 'boolean':
          ...
          break;
        default:
          throw new Error(`Unsupported JSON schema property type '${schemaProp.type}'`);
      }
    }
  ...
  compileSubSchema(subJsonSchema, propPrefix) {
    const modelDef = this.modelDefinition;
    const subDef = new JsonSchemaDefinition(subJsonSchema, null, this.options)
      .compileAllProps()
      .getModelDefinition();
    ...
    for (const subPropName in subDef) {
      if (subDef.hasOwnProperty(subPropName)) {
        ...
        modelDef[propName] = subDef[subPropName];
      }
    }
  }
